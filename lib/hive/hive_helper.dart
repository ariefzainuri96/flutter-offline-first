...
import '../features/download-master/model/kode_daerah_response.dart';
...
class HiveHelper {
  static initHive() async {
    ...
    await Hive.openBox<AddData>(HiveBoxes.addData);
  ...
  static saveUserToBox(AuthData authData) async {
    Box<AuthData> box = Hive.box(HiveBoxes.auth);
  ...
  static Future<void> saveKodeDaerah(
    String boxName,
    List<KodeDaerahResponse> data,
  ) async {
    List<KodeDaerahData> dataToSave =
        data.map((e) => KodeDaerahData(kode: e.kode, nama: e.nama)).toList();
    Box<KodeDaerahData> box = Hive.box(boxName);
    await box.clear();
    await box.addAll(dataToSave);
  }

  static List<KodeDaerahData> getKodeDaerah(String boxName) {
    Box<KodeDaerahData> box = Hive.box(boxName);
    return box.values.toList();
  }
